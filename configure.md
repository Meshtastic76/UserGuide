[Главная](README.md) > [Выбор](choose.md) > [Прошивка](flash.md)
...
## [LoRa](#LoRa)
Настройки устройства -> LoRa

| Параметр               | Значение                    | Комментарий                                          |
| ---------------------- | --------------------------- | ---------------------------------------------------- |
| Регион                 | Россия                      |                                                      |
| Использовать шаблон    | Вкл                         |                                                      |
| Шаблон                 | Средняя дальность - Быстрый | Medium fast                                          |
| ОК в MQTT              | Вкл                         | Выкл - если не хотите передавать данные в MQTT       |
| Передача включена      | Вкл                         |                                                      |
| Количество прыжков     | 4*                          | См. детали ниже                                      |
| Частота слота          | 1                           | Должно установиться автоматически при выборе шаблона |
| Переопределить частоту | 868.825                     | Должно установиться автоматически при выборе шаблона |
...
\*\* Для повышения мощности передачи можно сменить регион на EU_868

| Количество прыжков | Когда лучше установить |
| ------------------ | ---------------------- |
| 4 | Если вы находитесь в центре сети или на возвышенности и с хорошим приемом и передачей. **Этого должно хватать.** |
| 5 | Если вы находитесь на окраине сети (или в зоне плохого сигнала) и вас иногда спасает один лишний прыжок |
| 6 / 7 | Если ваш сигнал точно без промежуточной одной или двух нод не попадает в сеть (Так же если вы <br>в зоне плохого сигнала (например внутри здания) и используете связку CLIENT_MUTE -> CLIENT_BASE ) |
...
**О:** Не думайте, что увеличение прыжков поможет. Тут может быть несколько вариантов: 
...
## [Каналы](#Каналы)
Настройки устройства -> Каналы

| Параметр   | Значение | Комментарий                                                            |
| ---------- | -------- | ---------------------------------------------------------------------- |
| Имя канала | MediumFast | Должен установиться сам по умолчанию при выборе шаблона в разделе LoRa |
| PSK        | AQ==     | Должен установиться сам по умолчанию при выборе шаблона в разделе LoRa |

## [Пользователь](#Пользователь)
Настройки устройства -> Пользователь
...
Настройки устройства -> Устройство
...
| Интервал вещания информации об узле | Twelve Hours                       | 12 часов. <br>См. раздел "Настройка периодичности рассылки технических пакетов" |
| Часовой пояс                        | GMT-3                              |                                                                             |
...
(Если у вас непонятным образом отображается настройка временных интервалов рассылок, переключитесь на английский язык)

## [Информация о ноде](#Информация_о_ноде)
Настройки устройства -> Устройство -> Интервал вещания инфомации об узле
...
Настройки устройства -> Местоположение
...
**Как настроить местоположение вручную:**
Координаты, которые нужно вписать вручную берем здесь:
1. Заходим на сайт: https://yandex.ru/maps тыкаем правой кнопкой мыши в карту и нажимаем "Что здесь?". После этого в левом верхнем углу будут отображены координаты.
2. Чтобы установить высоту заходим еще на один [сайт](https://ru-ru.topographic-map.com/map-k2rb57/%D0%AF%D1%80%D0%BE%D1%81%D0%BB%D0%B0%D0%B2%D1%81%D0%BA%D0%B0%D1%8F-%D0%BE%D0%B1%D0%BB%D0%B0%D1%81%D1%82%D1%8C/?center=57.63805%2C39.91608&zoom=11&base=4). Тыкаем на карту, получаем значение высоты над уровнем моря. Потом прибавляем этаж на котором живем.
   Примерно так: Для этажа 3 высота будет такая: Высота топографическая + высота цоколя дома + высота потолков, умноженная на три 

## [Телеметрия](#Телеметрия)
Настройки модуля -> Телеметрия
...
## [MQTT](#MQTT) 
Настройки модуля -> MQTT

Не обязательная опция, если у вас нода с хорошей слышимостью, то можно помочь сообществу в сборе статистики, отправив пакеты на MQTT сервер.
...
## [Neighbor info config](#Neighbor) 
Настройка модуля -> Информация об окружности (Более корректный перевод "Информация о соседях")

Не обязательная опция, но для стационарных нод желательно включить.

Включение этой опции позволит автоматически построить топологию сети. Но так же это будет генерировать дополнительный технический трафик.
Чтобы не засорять эфир будем собирать статистику 1 раз в неделю. Причем, исходя из соображений, что было бы лучше, чтобы это происходило примерно в одно и то же время, сделаем это так:
В настройках интервала рассылки ставим 604800 секунд. Это будет ровно 7 дней. А чтобы срез данных был примерно в одно и то же время,
устанавливаем эту настройку в ночь со среды на четверг с 0:00 до 3:00 ночи (середина недели, эфир в это время должен быть относительно свободен). 
Выберите себе сами случайное время из этого диапазона, для установки таймера.

| Параметр               | Значение        | Комментарий                   |
| ---------------------- | --------------- | ----------------------------- |
| Информация о соседях включена            | Вкл              |            |
| Интервал обновления                      | 604800           |            |
| Передать через LoRa                      | Вкл              |            |



## ⏩ Следующий шаг
[Завершение](finish.md)
